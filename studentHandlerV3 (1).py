from StudentDataCenter import addStudent,modifyStudent,deleteStudent,getStudentById,getStudentByName
...
def add_student (userInput):
    ...
    if len(userInput)!=3:
    ...
    elif isValidName(userInput[1])and isValidDOB(userInput[2]):
        canAdd=True
        for dataDict in studentEntities:
            if dataDict["name"]==userInput[1]:
                print("Two student cannot have same name")
                canAdd=False
                break
        if canAdd:
            #nameList.append(userInput[1])
        ...
        if(userInput[1].isalnum() and userInput[-2].isalnum()):
...
def modify_student (userInput):
    ...
    if len(userInput)!=4:
        print("Invalid syntax press 'help-' to check valid command list")
    elif(isValidName(userInput[2])and isValidDOB(userInput[3])):
        selectedstudentDict=getStudentById(userInput[1])
        if selectedstudentDict==None:
            print("Invalid student Id")
        else:
            from MarksHandler import nameSubjectMarkList
            for dictStud in nameSubjectMarkList:
                if (dictStud["Name"])==(selectedstudentDict["name"]):
                    dictStud["Name"]=(userInput[2])
...
def remove_student (userInput):
    ...
    if len(userInput)!=2:
        ...
        selectedstudentDict=getStudentById(userInput[1])
        studName=selectedstudentDict["name"]
        if selectedstudentDict==None:
            print("Invalid student Id")
        else:
            from MarksHandler import nameSubjectMarkList
